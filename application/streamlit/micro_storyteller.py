...
创作准则："""故事行文流畅易于理解，尽量简短（2到4句话，约40到70个字符，最长不超过100个字符）。
通过故事内容来体现两个人的风格特点，而不要直接描述风格。
如果可以，希望有一些简单的反转，反转要与上文相关，复合逻辑。
...
结合你的的风格特点和环境，为以上创作场景写一个简短（2到4句话，约40到70个字符，最长不超过100个字符）的微故事。'''
        ...
        if mode == "遇见人":
            with st.expander("变量设置", True):
                ...
                area = st.text_input(label="area",
                                     value='商业中心',
                                     help="填写故事发生地点，如：商业中心，海边，集市，公园，生活区。也可以具体到某个子区域，如：商业中心的书店",
                                     key="area",
                                     on_change=init_messages)
                ...
                if "examples" not in st.session_state:
                    examples = zip([style_a] + random.sample(STLYES, 3), [style_b] + random.sample(STLYES, 3))
                    i = 0
                    examples_str = ''
                    for a, b in examples:
                        ...
                        if example not in examples_str:
                            i += 1
                            examples_str += f"示例{i}(你「{a}」和%s「{b}」)：" + EXAMPLES[STLYES.index(a)][
                                STLYES.index(b)] + '\n'
                        if i >= 3:
                            break
                    st.session_state.examples = examples_str.strip()

                examples = st.text_area("examples (故事示例)",
                                        value=st.session_state.examples,
                                        height=100,
                                        help="随机提供3个故事示例，如果不希望提供示例，可以将文本删除并提交",
                                        on_change=init_messages)
            # st.header("指令模版")
