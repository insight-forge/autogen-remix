...
class CustomGroupChatManager(ConversableAgent):
    ...
    def __init__(
        ...
        self.st = streamlit
        self.register_reply(Agent, CustomGroupChatManager.run_chat, config=groupchat,
                            reset_config=CustomGroupChat.reset)
    ...
    def run_chat(
        ...
        speaker = sender
        groupchat = config
        with self.st.chat_message(sender.name, avatar=HEAD_SCULPTURE[sender.name]):
        ...
        for i in range(groupchat.max_round):
            ...
            if self._is_termination_msg(message):
                self.st.session_state.manager = self
                self.st.session_state.user_proxy = sender
                return True, None
            ...
            try:
                ...
                self.st.toast(f"当前发言者：{speaker.name}")
            ...
            with self.st.chat_message(speaker.name, avatar=HEAD_SCULPTURE[speaker.name]):
                if 'name' in message:
                    if message['name'].startswith('image_'):
                        self.st.image(message['content'], width=350)
                    else:
                        self.st.markdown(message['content'])
                elif 'function_call' in message:
                    self.st.markdown(message['function_call'])
                else:
                    self.st.markdown(message['content'])
        self.st.session_state.manager = self
        self.st.session_state.user_proxy = sender
